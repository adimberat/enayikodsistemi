...
return message.channel.send(`Sen botun kara listesindesin!`)
  ...
  if(db.fetch(`botbakım`)) {
	let command2 = message.content.split(' ')[0].slice(prefix.length);
	  if(!message.content.startsWith(prefix)) return
	  
	  if(!client.commands.get(command2)) return
	  if(message.author.id === client.conf.sahip) {}
	  else {
		let embed31 = new Discord.MessageEmbed()
		.setTitle('Bakımdayız <a:unlem:730484050288181329>')
		.setDescription('Bot şu anda bakımda lütfen daha sonra tekrar dene!')
		.addField('Bakım Sebebi:', `${db.fetch(`botbakım`)}`)
		.setColor(client.conf.renk)
		 return message.channel.send(embed31).then(m => m.delete({timeout: 5000}))
	  }
  }
